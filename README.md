...
> 轻量 · 极速 · 纯本地 —— 图片查看、编辑、截图、取色一体化的 Windows 应用。
...
- **多种导入方式**：打开文件（图片 / PDF / OFD，可多选）、打开文件夹（含子文件夹递归）、拖拽文件/文件夹、`Ctrl+V` 粘贴截图、一键观看内置演示
...
- **图片编辑器**：裁剪（自由、1:1、4:3、16:9）、旋转、亮度/对比度/饱和度/模糊/图片不透明度调整；提供画笔、直线、矩形、椭圆、箭头、马赛克和文字标注，可拖动/删除标注；图形支持按角度旋转，文字可在图片任意位置输入并双击编辑；文字字号与绘图粗细/马赛克块共用控制，所有标注共用一套标准色卡，也可用取色器从屏幕任意位置取色并记住自定义颜色；支持自定义输出宽高、撤销、重做与重置，原文件不会被覆盖
...
- **屏幕取色**：顶栏「取色」一键抓取当前屏幕并进入全屏取色层，放大镜逐像素预览并实时显示 HEX / RGB 与坐标，点击取色后 HEX 自动复制到剪贴板，`Esc` 取消；编辑器调色板内也可直接取色作为绘图颜色
...
- **格式支持**：JPG / PNG / GIF / WebP / AVIF / BMP / ICO / SVG（跟随浏览器解码能力）；PSD / PSB 使用合成图预览，AI 使用 Illustrator 保存的 PDF 兼容内容预览，DWG 使用内嵌缩略图或 LibreDWG 矢量预览；PDF / OFD 文档按页渲染预览，查看器内 `←` / `→` 翻页
...
│   ├── formats.js  # PSD/PSB、PDF、DWG 延迟加载预览引擎
│   ├── ofd.js      # OFD 文档按页渲染预览
│   ├── vendor/     # 已内嵌的 PSD、PDF.js、LibreDWG WebAssembly 封装
│   ├── wasm/       # LibreDWG WebAssembly 运行时
│   ├── screenshot.js # 全屏截图裁剪控制器
│   ├── picker.js   # 全屏屏幕取色层：放大镜 / HEX+RGB 读数 / 剪贴板复制
│   ├── viewer.js   # 全屏查看器：缩放/平移/旋转/幻灯片/信息面板/文档翻页
